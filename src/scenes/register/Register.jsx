...
const Register = () => {
  ...
  const { SignUp,updateUserProfile, userGoogleLogin, setLoader } = useContext(UserAuthContext);
  const googleProvider = new GoogleAuthProvider();
  const navigate = useNavigate();
  const [signupError, setSignupError] = useState("");

 // Example in your registration component
const handleRegister = (data) => {
  SignUp(data.email, data.password)
    .then((result) => {
      const user = result.user;
      updateUserProfile(user, data.name, '', 'user'); // Pass name and role
      toast.success('Registration Successful');
      navigate('/');
      console.log(user);
    })
